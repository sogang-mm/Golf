...
while vid.isOpened():
    ...
    if cnt % sampling == 0:
        ...
        action_mask = cv2.bitwise_and(next_action_mask, prev_action_mask)
        m_cntrs, hierachy = cv2.findContours(action_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        for cntr in m_cntrs:
            #cv2.drawContours(image=visual, contours=[cntr], contourIdx=-1, color=(0, 255, 0), thickness=-1)
            cv2.drawContours(image=prev_next_action_mask, contours=[cntr], contourIdx=0, color=100, thickness=-1)
        ...
        motion[:, :, :3][action_mask != 0] = prev[action_mask != 0]
