...
'''
out_vid = cv2.VideoWriter('E:/golf-analyze/ppt/vid4.mp4',
...
while vid.isOpened():
    ...
    motion=cv2.addWeighted(prev_motion,.9,action,.7,0)
    #motion = cv2.add(motion, action, 0)
...
#out = cv2.addWeighted(last_frame, .9, start_frame,.9, 0)
out = cv2.addWeighted(start_frame, 0.5, motion, 0.9, 0)
cv2.imshow('out', out)
cv2.waitKey(0)
