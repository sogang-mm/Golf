...
while vid.isOpened():
    ...
    # segmentation
    seg_file = os.path.join(seg_path, 'result_{}.png'.format(frame_cnt+1))
    if os.path.exists(seg_file):
        seg = cv2.imread(seg_file, cv2.IMREAD_UNCHANGED)
        # in seg
        action_mask = cv2.bitwise_and(cv2.bitwise_not(seg[:, :, 3]), action_mask)
        #action_mask = action_mask-seg[:,:,3]
        cv2.imshow('segm', action_mask)
        cv2.imshow('aaaa',cv2.bitwise_and(frame,frame,mask=action_mask))
